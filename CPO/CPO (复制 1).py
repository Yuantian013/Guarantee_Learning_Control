...
class GuassianMLP_from_flatten_params(object):
    def __init__(self, s, a_dim, name, trainable, shape=[64,32],):
        ...
        with tf.variable_scope(name):
            ...
            base_distribution = tf.distributions.MultivariateNormalDiag(loc=self.mu, scale_diag=tf.exp(self.log_sigma))
...
class CPO(object):
    def __init__(self, a_dim, s_dim, args,
        ...
        with tf.name_scope('kl_divergence'):
            ...
            self.kl_mean = tf.reduce_mean(tfp.distributions.kl_divergence(pi, oldpi))
    ...
    def update(self, s, s_, returns, l_returns, advs, l_advs, a, old_values, old_l_values, l_reward, safety_eval,cliprangenow, LR_C, LR_L, length_of_trajectory):
        ...
        sub_feed_dict = {self.tfs: sub_s, self.tfa: sub_a, self.tfadv: sub_advs, self.tfdc_r: sub_returns, self.LR_C: LR_C, self.LR_L: LR_L,
                     self.S_: sub_s_,self.safety_gradient_rescale:[length_of_trajectory],
                         self.safety_threshold: threshold}
        if self.use_lyapunov:
